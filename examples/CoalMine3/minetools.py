...
def makePrimaryPotentialsWithRobinCondition(domain, minegeo, schedule, sigma_at_stations):
    sigma_ref=1.
...
def makeSecondaryPotentialsWithRobinCondition(domain, minegeo, sigma, sigma_faces, sigma_at_stations, primary_potentials, schedule):
    """
...
def makePrimaryPotentials(domain, minegeo, schedule, sigma_at_stations):
    sigma_ref=1.
    mask_face = MaskFromBoundaryTag(domain, *minegeo.TagFaces)
    primary_potentials = {}
    pde = setupERTPDE(domain, tolerance=1e-10)
    pde.setValue(A=sigma_ref * kronecker(3), y_dirac=Data(), X=Data(), Y=Data(), y=Data(), q=mask_face)
    for A in schedule.getListOfInjectionStations():
        iA=schedule.getStationNumber(A)
        s = Scalar(0., DiracDeltaFunctions(domain))
        s.setTaggedValue(f"s{A}", 1.)
...
def makeSecondaryPotentials(domain, minegeo, sigma, sigma_at_stations, primary_potentials, schedule):
    """
    It is assumed that sigma_ref=sigma on faces!!!!
    """
    mask_face = MaskFromBoundaryTag(domain, *minegeo.TagFaces)
    pde = setupERTPDE(domain, tolerance=1e-10)
    secondary_potentials = {}
    pde.setValue(A=sigma * kronecker(3), y_dirac=Data(), X=Data(), Y=Data(), y=Data(), q=mask_face)
    for iA in primary_potentials:
        sigma_ref = sigma_at_stations[iA]
        pde.setValue(X=(sigma_ref-sigma)*grad(primary_potentials[iA]))
        secondary_potentials[iA] = pde.getSolution()
        print(f"secondary potential for injection at {schedule.getKeyOfStationNumber(iA)}: {str(secondary_potentials[iA])}.")
    print(f"{len(secondary_potentials)} secondary potentials calculated.")
    return secondary_potentials
